...
const ActivityCU = (props)=>{
  // 작성페이지에서 사용되면 props.isEdit 에 false들어있음
  // 수정페이지에서 사용되면 props.isEdit 에 true 들어있음
  ...
  const handleChange =  (e)=>{
    // console.log(e);
    // e.target.name 이 'title' 이면 formInfo.values.title을 변경
    // e.target.name 이 'content' 이면 formInfo.values.content을 변경
    let cpy = JSON.parse(JSON.stringify(formInfo));
