...
app.post("/api/users", async (req, res) => {
  ...
  try {
    let [result, fields] = await pool.query(sql, [
      email,
      enPw,
      question,
      answer,
...
app.post("/api/login", async (req, res) => {
  ...
  // mysql에서 해당 email과 해당 password 존재하는지
  try {
...
app.post("/api/career", async (req, res) => {
  // email 은 header에 있는 token 에 들어있음
  const token = req.headers.authorization.replace("Bearer ", "");
  ...
  try {
    ...
    console.log(results);
    let [rows] = await pool.query("SELECT * FROM tbl_careers WHERE id=?", [
      results.insertId,
    ]);
    console.log(rows[0]);
...
// 좋아요 테이블에 추가 필요한 정보(리액트가 줘야하는 건 게시물id, 로그인한사람email)
app.post('/api/like', async(req, res)=>{
  const id = req.body.id; // 게시물 id 
  const token = req.headers.authorization.replace('Bearer ', '');
  const user = jwt.verify(token, process.env.JWT_SECRET);
  // user.email : 로그인한사람 email
...
// 좋아요 테이블에 삭제(리액트가 줘야하는 건 게시물id, 로그인한사람email)
...
app.post('/api/activities', upload.array('images') , async (req, res)=>{
  ...
  // req.files[0].filename --> 저장된 파일 이름
...
app.delete('/api/activities/:id', async (req, res)=>{
  ...
  try{
    ...
    let [rows] = await pool.query('select * from tbl_activities where id = ?' , [id]);
